...
// Only extend if ThreeGlobe is available
if (typeof window !== "undefined" && ThreeGlobe) {
  extend({ ThreeGlobe: ThreeGlobe });
}

const RING_PROPAGATION_SPEED = 2; // Reduced for better performance
const aspect = 1.2;
const cameraZ = 300;
...
export function Globe({ globeConfig, data }: WorldProps) {
  ...
  useEffect(() => {
    ...
    globeRef.current
      ...
      .pointsMerge(true)
      .pointAltitude(0.0)
      .pointRadius(1.5); // Reduced point radius
  ...
  // Handle rings animation with cleanup - reduced frequency
  useEffect(() => {
    if (!globeRef.current || !isInitialized || !data) return;
    ...
    const interval = setInterval(() => {
      ...
      const newNumbersOfRings = genRandomNumbers(
        0,
        data.length,
        Math.floor((data.length * 2) / 5), // Reduced ring count
      );
